...
namespace ElectionAlerts.Repository.RepositoryClasses
{
    public class VoterRepository : IVoterRepository
    {
        ...
        public IEnumerable<VoterPPBooth> GetDistinctPartNo(int roleid, int userid)
        {
            try
            {
               return _customContext.Set<VoterPPBooth>().FromSqlRaw("USP_GetPartNoBooth {0},{1}",roleid,userid).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
