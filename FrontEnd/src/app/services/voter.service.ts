...
export class VoterService {
  ...
  // all voter mobile file 

  voterWithMobileFile(UserId:any, RoleId:any, PageNo:any, NoofRow:any, Language:any): Observable<Blob>{
    const headers = new HttpHeaders({
      'Authorization': `Bearer ${this.token}`
    });
    return this.http.get(this.url+'Voter/VoterwithMobileNoFile?UserId='+UserId+'&RoleId='+RoleId+'&PageNo='+PageNo+'&NoofRow='+NoofRow+'&Language='+Language,{ responseType: "blob", reportProgress: true, headers })
  }
  ...
  getLandingImage(userId:number):Observable<Blob>{
    ...
    return this.http.get(this.url+'Voter/GetAllLandingPage?UserId='+userId,{responseType: "blob",headers})
  }
  ...
  slipOnWhatsapp(mobileNo:any, message:any, media_url:any, fileName:any, instance_id:any, access_token:any){
    debugger;
    const headers = new HttpHeaders();
    return this.http.get<any>('https://cloud.connects.digital/api/send?number='+mobileNo+'&type=media&message='+message+'&media_url='+media_url+'&filename='+fileName+'&instance_id='+instance_id+'&access_token='+access_token,{headers})
  }
